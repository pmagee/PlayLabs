...
if (newUserForm.hasErrors()) {
    ...
    return badRequest(addCustomer.render(newUserForm,newAddressForm,User.getUserById(session().get("email"))));
} else {
...
public Result updateCustomer(String email) {
    ...
    // Display the "add item" page, to allow the user to update the item
    return ok(addCustomer.render(userForm,aForm,User.getUserById(session().get("email"))));
}
